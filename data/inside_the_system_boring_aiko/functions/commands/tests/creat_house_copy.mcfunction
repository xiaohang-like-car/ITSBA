...
#-[!]此坐标会放一个箱子，里面有物品，玩家可以打开拿走，且会一段时间后更新（箱子的位置会生成一个带“aikos_chest”的标签的盔甲架）
...
execute if score server/test/hcstage config matches 1220 run execute at @s run summon armor_stand ~-1 ~ ~1 {Tags:["aikos_chest"],Invisible:1b,Invulnerable:1b,NoGravity:1b}
#-结束标记
#
...
execute if score server/test/hcstage config matches 10000 run execute at @s run setblock ~-1 ~1 ~3 minecraft:oak_wall_sign[facing=south]{front_text:{color:"black",has_glowing_text:0b,messages:['{"text":""}','{"text":"Sweet house"}','{"text":""}','{"text":""}']},is_waxed:1b} destroy
execute if score server/test/hcstage config matches 10000 run playsound minecraft:block.wood.place voice @a ~ ~ ~ 100 1 1
